    ...
    if (missing(algorithms)) {
      ...
      if (missing(params)) {
        ...
        if (length(intersect(algorithms, c("MLeNN", "MLTL","MLSOL","MLUL"))) > 0) {
          ...
          neighbors2 <- getAllNeighbors2(neighbors, unique(unlist(.mldrApplyFun1(D$labels[D$labels$IRLbl < D$measures$meanIR,]$index, function(x) { c(1:D$measures$num.instances)[D$dataset[x]==1] }, mc.cores=.numCores))), max(params[params[,1] %in% c("MLeNN","MLTL"),3]), tableVDM)
      ...
      if (length(intersect(algorithms, c("MLeNN", "MLTL","MLSOL","MLUL"))) > 0) {
        ...
        startTime <- Sys.time()
        neighbors <- getAllNeighbors(D, c(1:D$measures$num.instances)[D$dataset$.labelcount > 0], tableVDM)
        neighbors2 <- getAllNeighbors2(neighbors, unique(unlist(.mldrApplyFun1(D$labels[D$labels$IRLbl < D$measures$meanIR,]$index, function(x) { c(1:D$measures$num.instances)[D$dataset[x]==1] }, mc.cores=.numCores))), k, tableVDM)
